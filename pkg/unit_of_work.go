...
func (uow *UnitOfWork) Generate(templateFile string, output string) {
	...
	embeddedTemplate := strings.ToLower(templateFile)

	var (
		tmpl              *template.Template
		templateFileParts []string
		templateName      string
		err               error
	)

	switch embeddedTemplate {
	case "docker-compose":
		tmpl, err = template.New(embeddedTemplate).Parse(tmplDockerCompose)
	case "crane":
		tmpl, err = template.New(embeddedTemplate).Parse(tmplCrane)
	case "systemd":
		tmpl, err = template.New(embeddedTemplate).Parse(tmplSystemD)
	case "upstart":
		tmpl, err = template.New(embeddedTemplate).Parse(tmplUpStart)
	case "dot":
		tmpl, err = template.New(embeddedTemplate).Parse(tmplDot)
	case "service-unit":
		tmpl, err = template.New(embeddedTemplate).Parse(tmplServiceUnit)
	default:
