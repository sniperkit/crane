...
func TestInitialize(t *testing.T) {
	...
	assert.Equal(t, map[string][]string{"default": {"a", "b"}}, c.groups)
...
func TestInitializeAmbiguousHooks(t *testing.T) {
	...
	rawHooksMap := map[string]hooks{
		"group1": {RawPreStart: "group1-pre-start"},
		"group2": {RawPreStart: "group2-pre-start"},
	}
	c := &config{
		RawContainers: rawContainerMap,
		RawGroups:     rawGroups,
...
func TestValidate(t *testing.T) {
	rawContainerMap := map[string]*container{
		"a": {RawName: "a", RawImage: "ubuntu"},
		"b": {RawName: "b", RawImage: "ubuntu"},
	}
